...
def plot_accels_forfig(fig, plotnum, trial, tracklist):
...
def plot_analysis_forfig(fig, rows, columns, number, finbeats_subset,
    ...
    Args:
        fig (matplotlib figure): the name of the figure
    ...
    # Set up the figure and choose an appropriate z-axis label
    ax1 = fig.add_subplot(rows, columns, number, projection='3d')
    ax1.set_xlabel('Period (s)')
    ax1.set_ylabel('Amplitude (cm)')
    if zaxis == 'V':
...
def plot_fft_forfig(fig, rows, columns, number,
                    trial, tracklist):
    ...
    ax = fig.add_subplot(rows, columns, number)
...
def plot_fit_sine_forfig(fig, nrows, ncols, number, trial, tracklist,
                         sin_corr_df):
    """Plots detrended tail tip motion against best fit sine wave
    ...
    ax = fig.add_subplot(nrows, ncols, number)
    ax.set_xlabel('Time (s)')
    ax.set_ylabel('Position (cm)')
...
def plot_trace_forfig(fig, rows, columns, number, trial, tracklist):
...
def plot_traj_forfig(fig, plotnum, trial, tracklist, finbeat_subset):
    """Plots a single trial's finbeats against steady finbeats 
    This function is used within a figure script to produce panels 
    ...
    ax1 = fig.add_subplot(plotnum, projection='3d')
    # fig.suptitle(trial +' '+ behavior)
    for a, b, c in steady_finbeats:
